  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="p-4 border-t border-[#334155] space-y-3">
        <div className="flex gap-1.5 bg-[#0f172a] rounded-xl p-1 border border-[#334155]">
          {['en', 'uk', 'de', 'it'].map(lang => (
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${uiLang === lang ? 'bg-[#6366f1] text-white' : 'text-[#475569] hover:text-white'}`}
            >
              {lang}
            </button>
          ))}
        ...
        {!planLoading && !isPro && (
          <button onClick={() => setShowUpgrade(true)} className="w-full py-2.5 px-4 text-xs font-black text-white bg-[#6366f1] hover:bg-[#4f46e5] rounded-xl transition-all shadow-lg shadow-[#6366f1]/20">
            ✦ Upgrade to Pro
          </button>
        )}
        {planLoading && (
          <div className="w-full py-2.5 px-4 text-xs font-black text-[#475569] bg-[#1e293b] rounded-xl text-center animate-pulse">Loading...</div>
        )}
        <button onClick={logout} className="w-full py-2.5 px-4 text-xs font-semibold text-[#64748b] hover:text-[#94a3b8] border border-[#334155] rounded-xl hover:border-[#475569] transition-all">
          {dict.logout}
        </button>
      </div>
    </>
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-none mx-2 flex-1" style={{ scrollbarWidth: 'none' }}>
          {navItems.map(item => (
            <button
              key={item.id}
              onClick={() => handleNav(item.id)}
              className={`shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all ${activeTab === item.id ? 'bg-[#6366f1] text-white' : 'text-[#475569] hover:text-white'}`}
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative z-10 w-72 max-w-[85vw] bg-[#1e293b] flex flex-col h-full shadow-2xl animate-slide-in-left">
            <div className="flex items-center justify-between px-5 pt-5 pb-0">
              ...
              <button onClick={() => setMobileOpen(false)} className="w-8 h-8 flex items-center justify-center text-[#64748b] hover:text-white rounded-lg hover:bg-[#334155] transition-all text-xl">✕</button>
            </div>
            <SidebarContent />
          </aside>
        </div>
