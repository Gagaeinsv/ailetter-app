...
const MobileHistoryTab = ({ history, setHistory, setGeneratedLetter, setActiveTab, dict, showNotification }) => {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#0f172a] px-4 py-6 pb-24 custom-scrollbar">
      {/* ВИПРАВЛЕНО: dict?.history замість "Історія листів" */}
      <h2 className="text-2xl font-black text-white mb-6">{dict?.history || "History"}</h2>
      
      <div className="flex gap-3 mb-6">
          <div className="relative flex-1">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"><IconSearch /></span>
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="w-full bg-[#1e293b] border border-[#334155] rounded-xl py-3 pl-9 pr-3 text-sm text-white focus:border-[#6366f1] outline-none" />
          </div>
          <div className="flex bg-[#1e293b] rounded-xl p-1 border border-[#334155] shrink-0">
              {['all', '7'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${filter === f ? 'bg-[#6366f1] text-white' : 'text-gray-400 hover:text-white'}`}>
                      {f === 'all' ? 'All' : '7d'}
                  </button>
              ))}
          </div>
      ...
      <div className="space-y-4">
        {getFilteredHistory().length === 0 ? (
           <div className="text-gray-500 text-center py-10 text-sm"><p>No history found</p></div>
        ) : (
           getFilteredHistory().map(item => (
              <div key={item.id} className="bg-[#1e293b] p-4 rounded-xl border border-[#334155] shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                      ...
                      <button onClick={() => handleDelete(item.id)} className="text-gray-500 hover:text-red-400 p-1"><IconTrash /></button>
                  </div>
                  <div className="flex gap-2 border-t border-[#334155] pt-3">
                      <button onClick={() => { setGeneratedLetter(item.text); setActiveTab('dashboard'); }} className="flex-1 py-2 bg-[#6366f1]/10 text-[#6366f1] rounded-lg text-xs font-bold uppercase">Load</button>
                      <button onClick={() => handleDuplicate(item)} className="p-2 bg-[#0f172a] text-gray-400 rounded-lg border border-[#334155]"><IconCopy /></button>
                  </div>
              </div>
           ))
        )}
