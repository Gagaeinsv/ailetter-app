...
const STEPS = [
  { key: 'profession', label: 'Хто ти за фахом?',      placeholder: 'Python Developer',              type: 'text'     },
  { key: 'experience', label: 'Досвід роботи (роки)',   placeholder: '2',                             type: 'number'   },
  { key: 'skills',     label: 'Основні навички',        placeholder: 'React, AWS, Machine Learning...', type: 'textarea' },
];
  ...
  const handleSubmit = async () => {
    ...
    try {
      await setDoc(doc(db, 'users', auth.currentUser.uid), {
        ...
        setupComplete: true,
        updatedAt:     new Date(),
      }, { merge: true }); // merge: true — не перезаписує plan і інші поля
    ...
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-4 text-white font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-sm md:max-w-md relative z-10">
        ...
        <div className="bg-[#1e293b] border border-[#334155] rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-2xl">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-indigo-400 mb-1 text-center">Твій профіль</h2>
          <p className="text-[#64748b] text-xs text-center mb-7">Ці дані допоможуть AI писати листи від твого імені</p>
          ...
          <div className="flex gap-3 mt-6">
            ...
            >
              {loading ? 'Saving...' : isLast ? '✓ Завершити' : 'Далі →'}
            </button>
          </div>
        </div>
