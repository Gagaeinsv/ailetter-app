...
const Dashboard = () => {
  ...
  // Після повернення з Stripe — polling Firestore до появи Pro статусу
  useEffect(() => {
    if (window.location.search.includes('success=true')) {
      window.history.replaceState({}, '', window.location.pathname);

      // Перевіряємо план кожні 2 секунди до 30 секунд
      let attempts = 0;
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          const verifyPlan = httpsCallable(fns, 'verifyPlan');
          const result = await verifyPlan();
          if (result.data.isPro) {
            clearInterval(interval);
            showNotification('🎉 Welcome to Pro! All features unlocked.');
          }
        } catch (e) {
          // silent — usePlan Firestore listener оновить UI автоматично
        }
        if (attempts >= maxAttempts) clearInterval(interval);
      }, 2000);

      // Показуємо нотифікацію одразу
      showNotification('✅ Payment successful! Activating Pro...');
    }
